...
def calcSimlaryCosDist(user1,user2):
    ...
    for key in user1:
        avg_x+=key[1]
    avg_x=avg_x/len(user1)#user1对看过的电影的平均分

    for key in user2:
        avg_y+=key[1]
    avg_y=avg_y/len(user2)#user2对看过的电影的平均分

    for key1 in user1:
        for key2 in user2:
            if key1[0]==key2[0] :
                sum_xy+=(key1[1]-avg_x)*(key2[1]-avg_y)#对于相同电影，（user1的评分-user1的平均分）*（user2的评分-user2的平均分）
                sum_y+=(key2[1]-avg_y)*(key2[1]-avg_y)#对于相同电影，（user2的评分-user2的平均分）*（user2的评分-user2的平均分）
        sum_x+=(key1[1]-avg_x)*(key1[1]-avg_x)#对于相同电影，（user1的评分-user1的平均分）*（user1的评分-user1的平均分）
...
def recommendByUserFC(file_name,userid,k=5):
    ...
    user_dic,moive_to_user=createUserRankDic(test_rates)

    #寻找邻居
    neighbors=calcNearestNeighbor(userid,user_dic,moive_to_user)[:k]

    recommend_dic={}#{所有看过这部电影用户的相似度之和：电影名}
    for neighbor in neighbors:
        neighbor_user_id=neighbor[1]
        movies=user_dic[neighbor_user_id]
        #先找到邻居看过的电影，在遍历其余观看该邻居看过的电影的用户，将这些用于与目标用户的相似度求和
        for movie in movies:
            #print movie
    ...
    #建立推荐列表,转换为list
    recommend_list=[]#[所有看过这部电影用户的相似度之和，电影名]
    ...
    user_movies = [ i[0] for i in user_dic[userid]]

    return [i[1] for i in recommend_list],user_movies,moive_to_user,neighbors
